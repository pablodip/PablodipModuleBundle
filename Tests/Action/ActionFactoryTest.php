...
use Pablodip\ModuleBundle\Action\ActionFactory;

class ActionFactoryTest extends \PHPUnit_Framework_TestCase
{
    public function testFactory()
    {
        $action1 = $this->getMock('Pablodip\ModuleBundle\Action\ActionInterface');
        $action1->expects($this->any())->method('getFullName')->will($this->returnValue('action1'));

        $action2 = $this->getMock('Pablodip\ModuleBundle\Action\ActionInterface');
        $action2->expects($this->any())->method('getFullName')->will($this->returnValue('action2'));

        $action3 = $this->getMock('Pablodip\ModuleBundle\Action\ActionInterface');
        $action3->expects($this->any())->method('getFullName')->will($this->returnValue('action3'));

        $action4 = $this->getMock('Pablodip\ModuleBundle\Action\ActionInterface');
