...
use Pablodip\ModuleBundle\Action\ActionCollectionFactory;

class ActionCollectionFactoryTest extends \PHPUnit_Framework_TestCase
{
    public function testFactory()
    {
        $collection1 = $this->getMock('Pablodip\ModuleBundle\Action\ActionCollectionInterface');
        $collection1->expects($this->any())->method('getName')->will($this->returnValue('collection1'));

        $collection2 = $this->getMock('Pablodip\ModuleBundle\Action\ActionCollectionInterface');
        $collection2->expects($this->any())->method('getName')->will($this->returnValue('collection2'));

        $collection3 = $this->getMock('Pablodip\ModuleBundle\Action\ActionCollectionInterface');
        $collection3->expects($this->any())->method('getName')->will($this->returnValue('collection3'));

        $collection4 = $this->getMock('Pablodip\ModuleBundle\Action\ActionCollectionInterface');
